...
CONTAINS
  ...
  SUBROUTINE behave(Xpar,Ypar,Zpar,Pwc_zb,Pwc_zc,Pwc_zf,P_zb,P_zc,P_zf,        &
    ...
    if(P_age .GE. 14.0*86400) P_swim(n,3) = swimfast !GaBy after 14 days swim at max speed

    !   ***************** Prepare for TYPE 4 & 5 (Oyster Larvae) Behaviors
    
    !Update pediveliger behavior/status and timer
    IF(P_behave(n) .EQ. 8 .OR. P_behave(n) .EQ. 9) THEN
    ...
    IF (P_behave(n).EQ.1) THEN 
       ...
       !particle has 80% chance o`f swimming up if deeper than 1.0 m of surface
 ...
 !TYPE 8. LOPHELLIA  GaBy
    IF ((P_behave(n).EQ.8)) THEN             
       if(P_age .LT. 32*86400) then !nearsurface behaviour until...
	   btest = 0   !switch to control behavior
     
	       !particle has 80% chance of swimming up if deeper than 20 m of bottom
	       if (P_zc .LT. (P_zetac-20.0)) then
		  negpos = 1.0
		  dev1=genrand_real1()
		  switch = 0.80 
		  if (dev1.GT.switch) negpos = -1.0
		  ...
		  btest = 1
	       ...
	       !if within 20 m of surface, swim randomly (50% chance of swimming up)
	       if (btest.EQ.0) then    !si no entro arriba entra aqui...
		  negpos = 1.0
		  dev1=genrand_real1()
		  switch = 0.5 
		  if (dev1.GT.switch) negpos = -1.0
       ...
       elseif(P_age .GT. 32*86400) then !nearbottom? behaviour until dead
	   btest2 = 0   !switch to control behavior
 ...
 !TYPE 9. MYTILUS  GaBy
    IF ((P_behave(n).EQ.9)) THEN             
       if(P_age .LT. 20*86400) then !nearsurface behaviour until...
	   btest = 0   !switch to control behavior
       ...
       elseif(P_age .GT. 20*86400) then !nearbottom? behaviour until dead
	   btest2 = 0   !switch to control behavior
