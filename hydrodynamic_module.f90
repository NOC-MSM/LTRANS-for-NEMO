...
!  Last Modified on:         Feb 2013
  ...
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: E3T,E3W,dept,depw !NEMO!!!
  ...
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:)  :: m_r,m_u,m_v,E3T0e,E3W0e 
  INTEGER, ALLOCATABLE, DIMENSION(:,:) :: mask_rho, mbathy,bathy 

  INTEGER, ALLOCATABLE, DIMENSION( : ) :: rho_mask,u_mask,v_mask,bathym !ewn.v.2
...
CONTAINS
  ...
  SUBROUTINE initGrid()
    ...
    INTEGER :: i,j,m,k,count,inele,row,col
    ...
    ALLOCATE(bathy(4,rho_elements)) !NEMO
    ...
    ALLOCATE(x_v(vi,vj))
    ALLOCATE(y_v(vi,vj))    
    ALLOCATE(mbathy(vi,uj))  !For NEMO
    ALLOCATE(E3T0(vi,uj,us))  !For NEMO
    ALLOCATE(E3W0(vi,uj,ws))  !For NEMO
    ALLOCATE(rho_mask(rho_nodes))  !ewn.v.2
    ALLOCATE(bathym(rho_nodes))  !For NEMO
    ALLOCATE(E3T0e(rho_nodes,us))  !For NEMO
    ALLOCATE(E3W0e(rho_nodes,ws))  !For NEMO
    ALLOCATE(E3W(ws))  !!!!For NEMO
    ALLOCATE(E3T(us))  !!!!For NEMO    
    ALLOCATE(dept(ws))  !!!!For NEMO
    ALLOCATE(depw(us))  !!!!For NEMO    
    ...
    if (STATUS .NE. NF90_NOERR) write(*,*) NF90_STRERROR(STATUS)
      ...
      ! Number of Levels NEMO
      STATUS = NF90_INQ_VARID(NCID,'mbathy',VID)
      if (STATUS .NE. NF90_NOERR) write(*,*) 'Problem find mbathy'
      if (STATUS .NE. NF90_NOERR) write(*,*) NF90_STRERROR(STATUS)
      STATUS = NF90_GET_VAR(NCID,VID,mbathy)
      if (STATUS .NE. NF90_NOERR) write(*,*) 'Problem read mbathy'
      if (STATUS .NE. NF90_NOERR) write(*,*) NF90_STRERROR(STATUS)
      ...
      ! e3w_0 value
      STATUS = NF90_INQ_VARID(NCID,'e3w_0',VID)
      if (STATUS .NE. NF90_NOERR) write(*,*) 'Problem find e3w_0'
      if (STATUS .NE. NF90_NOERR) write(*,*) NF90_STRERROR(STATUS)
      STATUS = NF90_GET_VAR(NCID,VID,E3W0)
      if (STATUS .NE. NF90_NOERR) write(*,*) 'Problem read e3w_0'
      if (STATUS .NE. NF90_NOERR) write(*,*) NF90_STRERROR(STATUS)

      ! e3t_0 value
      STATUS = NF90_INQ_VARID(NCID,'e3t_0',VID)
      if (STATUS .NE. NF90_NOERR) write(*,*) 'Problem find e3t_0'
      if (STATUS .NE. NF90_NOERR) write(*,*) NF90_STRERROR(STATUS)
      STATUS = NF90_GET_VAR(NCID,VID,E3T0)
      if (STATUS .NE. NF90_NOERR) write(*,*) 'Problem read e3t_0'
      if (STATUS .NE. NF90_NOERR) write(*,*) NF90_STRERROR(STATUS)
...
!    write(*,*) TRIM(filenm)
    ...
    if (STATUS .NE. NF90_NOERR) write(*,*) NF90_STRERROR(STATUS)
      ...
      ! e3t value
!      STATUS = NF90_INQ_VARID(NCID,'e3t',VID)
!      if (STATUS .NE. NF90_NOERR) write(*,*) 'Problem find e3t'
 !     if (STATUS .NE. NF90_NOERR) write(*,*) NF90_STRERROR(STATUS)
  !    STATUS = NF90_GET_VAR(NCID,VID,E3T)
   !   if (STATUS .NE. NF90_NOERR) write(*,*) 'Problem read E3T'
    !  if (STATUS .NE. NF90_NOERR) write(*,*) NF90_STRERROR(STATUS)

      ! e3w value
     ! STATUS = NF90_INQ_VARID(NCID,'e3w',VID)
     ! if (STATUS .NE. NF90_NOERR) write(*,*) 'Problem find e3w'
     ! if (STATUS .NE. NF90_NOERR) write(*,*) NF90_STRERROR(STATUS)
     ! STATUS = NF90_GET_VAR(NCID,VID,E3W)
     ! if (STATUS .NE. NF90_NOERR) write(*,*) 'Problem read E3W'
     ! if (STATUS .NE. NF90_NOERR) write(*,*) NF90_STRERROR(STATUS)
    ...
    !Done below as for romsdepth
    ! For NEMO Assign mask values to mbathy nodes (Elementizar?)
   ! count = 0
   ! do j=1,uj
   !   do i=1,ui
    !    count = count + 1   !move to next node number
  !          !cycles through each variable replacing the vi,uj part with count
            !  essentially giving it node numbers
  !       bathym(count) = mbathy(i,j)* m_r(i,j)
 !     enddo
 !   enddo


!!!WRONG!!! NEEDS TO BE HYBRID BETWEEN THIS AND THE STUFF FOR TEMP (BELOW)!!!!
  !Reshape input to fit node numbers assigned to elements
    ! For NEMO Assign mask values to E3T0 nodes (Elementizar?)
    count = 0
    do j=1,uj
      do i=1,ui
        count = count + 1   !move to next node number
            ...
            !  essentially giving it node numbers 
	do k=1,us
        E3T0e(count,k) = E3T0(i,j,k)* m_r(i,j)
	enddo
      enddo
    enddo

   ! For NEMO Assign mask values to E3W0 nodes (Elementizar?)
    count = 0
    do j=1,uj
      do i=1,ui
        count = count + 1   !move to next node number
            ...
            !  essentially giving it node numbers
        do k=1,us
	E3W0e(count,k) = E3W0(i,j,k)* m_r(i,j)
	enddo
      enddo
    enddo
    ...
    do j=1,uj
      do i=1,vi
        ...
        bathym(count) = mbathy(i,j) !NEEDS TO BE BLANKED because ele is based on Wet elements only
	!done below like for  r_ele_x(i,j) = rx(RE(i,j)) 
      enddo
    enddo
  ...
  SUBROUTINE initHydro()
    ...
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:,:,:) :: romW,romKH,romS,romT,romD,romU,   &
                                             romV,nromW,nromKH,nromS,nromT,nromD,nromU,nromV
   ...
   !ALLOCATE NEMO format VARS to read in
    ALLOCATE(nromW(vi,uj,ws,3))
    ALLOCATE(nromS(vi,uj,us,3))
    ALLOCATE(nromT(vi,uj,us,3))
    ALLOCATE(nromD(vi,uj,us,3))
    ALLOCATE(nromU(ui,uj,us,3))
    ALLOCATE(nromV(vi,vj,us,3))
    ALLOCATE(nromKH(vi,uj,ws,3))
...
!       write(*,*) 'ncid = ', ncid
      ...
      if(readZeta)then  
        ...
        STATUS = NF90_INQ_VARID(NCID,'zos',VID)
        if (STATUS .NE. NF90_NOERR) then
      ...
      if(readSalt)then
        ...
        countr(3)=us
        countr(4)=3
        STATUS = NF90_INQ_VARID(NCID,'so',VID)
        if (STATUS .NE. NF90_NOERR) then
        ...
        STATUS = NF90_GET_VAR(NCID,VID,nromS(t_ijruv(1):t_ijruv(2),             &
	...
	write(*,*) 'COUNTr'
 	write(*,*) COUNTr
      ...
      if(readTemp)then  
        ...
        countr(3)=us
        countr(4)=3
        STATUS = NF90_INQ_VARID(NCID,'thetao',VID)
        if (STATUS .NE. NF90_NOERR) then
        ...
        STATUS = NF90_GET_VAR(NCID,VID,nromT(t_ijruv(1):t_ijruv(2),             &
                              t_ijruv(3):t_ijruv(4),:,:),STARTr,COUNTr)
	do l=1,us !Flip over NEMO
        romT(:,:,l,:)=nromT(:,:,(us+1)-l,:)
	enddo
      ...
      if(readDens)then  
        ...
        STATUS = NF90_GET_VAR(NCID,VID,nromD(t_ijruv(1):t_ijruv(2),             &
                              t_ijruv(3):t_ijruv(4),:,:),STARTr,COUNTr)
	do l=1,us !Flip over NEMO
        romD(:,:,l,:)=nromD(:,:,(us+1)-l,:)
	enddo
      ...
      if(readU)then  
        ...
        countr(3)=us
        countr(4)=3
        STATUS = NF90_INQ_VARID(NCID,'uo',VID)
        if (STATUS .NE. NF90_NOERR) then
        ...
        STATUS = NF90_GET_VAR(NCID,VID,nromU(t_ijruv(5):t_ijruv(6),             &
	...
	!write(*,*) 'U'
 	!write(*,*) romU
      ...
      if(readV)then  
        ...
        countr(3)=us
        countr(4)=3
        STATUS = NF90_INQ_VARID(NCID,'vo',VID)
        if (STATUS .NE. NF90_NOERR) then
	...
	!write(*,*) 'V'
 	!write(*,*) romV
      ...
      if(readW)then  
        ...
        countr(3)=ws
        countr(4)=3
        STATUS = NF90_INQ_VARID(NCID,'wo',VID)
        if (STATUS .NE. NF90_NOERR) then
        ...
        STATUS = NF90_GET_VAR(NCID,VID,nromW(t_ijruv(1):t_ijruv(2),             &
                              t_ijruv(3):t_ijruv(4),:,:),STARTr,COUNTr)
	do l=1,ws !Flip over NEMO
        romW(:,:,l,:)=nromW(:,:,(ws+1)-l,:)
	enddo
	!write(*,*) 'W'
 	!write(*,*) romW
        if (STATUS .NE. NF90_NOERR) then
          write(*,*) 'Problem read wo=w array'
          write(*,*) NF90_STRERROR(STATUS)
          stop
        endif
      else
      ...
      if(readAks)then  
        ...
        countr(3)=ws
        countr(4)=3
        STATUS = NF90_INQ_VARID(NCID,'avm',VID)
        if (STATUS .NE. NF90_NOERR) then
          write(*,*) 'Problem find avm=AKs'
          write(*,*) NF90_STRERROR(STATUS)
          stop
        endif

        STATUS = NF90_GET_VAR(NCID,VID,nromKH(t_ijruv(1):t_ijruv(2),            &
                              t_ijruv(3):t_ijruv(4),:,:),STARTr,COUNTr)
	do l=1,ws !Flip over NEMO
        romKH(:,:,l,:)=nromKH(:,:,(ws+1)-l,:)
	enddo
        if (STATUS .NE. NF90_NOERR) then
    ...
    !DEALLOCATE SUBROUTINE VARIABLES
    DEALLOCATE(romZ,romW,romD,romKH,romS,romT,romU,romV,nromW,nromD,nromKH,nromS,nromT,nromU,nromV)
  ...
  SUBROUTINE updateHydro()
    ...
    !Allocate NEMO format variables (fliped sigma)
    ALLOCATE(nromSf(vi,uj,us,1))
    ALLOCATE(nromTf(vi,uj,us,1))
    ALLOCATE(nromDf(vi,uj,us,1))
    ALLOCATE(nromUf(ui,uj,us,1))
    ALLOCATE(nromVf(vi,vj,us,1))
    ALLOCATE(nromWf(vi,uj,ws,1))
    ALLOCATE(nromKHf(vi,uj,ws,1))
...
!   write(*,*) 'ncid = ', ncid
    ...
    if(readU)then
      ...
      STATUS = NF90_GET_VAR(NCID,VID,nromUf(t_ijruv(5):t_ijruv(6),              &
                            t_ijruv(7):t_ijruv(8),:,:),STARTr,COUNTr)
	do l=1,us !Flip over NEMO
        romUf(:,:,l,:)=nromUf(:,:,(us+1)-l,:)
	enddo
	!write(*,*) 'Uf'
 	!write(*,*) romUf
      if (STATUS .NE. NF90_NOERR) then
    ...
    if(readV)then
      ...
      STATUS = NF90_GET_VAR(NCID,VID,nromVf(t_ijruv(9):t_ijruv(10),             &
                            t_ijruv(11):t_ijruv(12),:,:),STARTr,COUNTr)
	do l=1,us !Flip over NEMO
        romVf(:,:,l,:)=nromVf(:,:,(us+1)-l,:)
	enddo
	!write(*,*) 'Vf'
 	!write(*,*) romVf
      if (STATUS .NE. NF90_NOERR) then
    ...
    if(readSalt)then
      ...
      STATUS = NF90_GET_VAR(NCID,VID,nromSf(t_ijruv(1):t_ijruv(2),              &
                            t_ijruv(3):t_ijruv(4),:,:),STARTr,COUNTr)
	do l=1,us !Flip over NEMO
        romSf(:,:,l,:)=nromSf(:,:,(us+1)-l,:)
	enddo
      if (STATUS .NE. NF90_NOERR) then
        write(*,*) 'Problem read salt array'
        write(*,*) NF90_STRERROR(STATUS)
    ...
    if(readTemp)then
      ...
      STATUS = NF90_GET_VAR(NCID,VID,nromTf(t_ijruv(1):t_ijruv(2),              &
                            t_ijruv(3):t_ijruv(4),:,:),STARTr,COUNTr)
	do l=1,us !Flip over NEMO
        romTf(:,:,l,:)=nromTf(:,:,(us+1)-l,:)
	enddo
      if (STATUS .NE. NF90_NOERR) then
        write(*,*) 'Problem read temp array'
        write(*,*) NF90_STRERROR(STATUS)
    ...
    if(readDens)then
      ...
      STATUS = NF90_GET_VAR(NCID,VID,nromDf(t_ijruv(1):t_ijruv(2),              &
                            t_ijruv(3):t_ijruv(4),:,:),STARTr,COUNTr)
	do l=1,us !Flip over NEMO
        romDf(:,:,l,:)=nromDf(:,:,(us+1)-l,:)
	enddo
      if (STATUS .NE. NF90_NOERR) then
        write(*,*) 'Problem read rho array'
        write(*,*) NF90_STRERROR(STATUS)
    ...
    if(readW)then
      ...
      STATUS = NF90_GET_VAR(NCID,VID,nromWf(t_ijruv(1):t_ijruv(2),              &
                            t_ijruv(3):t_ijruv(4),:,:),STARTr,COUNTr)
	do l=1,ws !Flip over NEMO
        romWf(:,:,l,:)=nromWf(:,:,(ws+1)-l,:)
	enddo
      if (STATUS .NE. NF90_NOERR) then
    ...
    if(readAks)then
      ...
      STATUS = NF90_INQ_VARID(NCID,'avm',VID)
      if (STATUS .NE. NF90_NOERR) then
        write(*,*) 'Problem find avm=AKs f'
        write(*,*) NF90_STRERROR(STATUS)
        stop
      endif
      STATUS = NF90_GET_VAR(NCID,VID,nromKHf(t_ijruv(1):t_ijruv(2),             &
                            t_ijruv(3):t_ijruv(4),:,:),STARTr,COUNTr)
	do l=1,ws !Flip over NEMO
        romKHf(:,:,l,:)=nromKHf(:,:,(ws+1)-l,:)
	enddo
      if (STATUS .NE. NF90_NOERR) then
        write(*,*) 'Problem read AKs array'
        write(*,*) NF90_STRERROR(STATUS)
    ...
    !DEALLOCATE SUBROUTINE VARIABLES
    DEALLOCATE(romZf,romSf,romTf,romUf,romVf,romWf,romKHf,nromWf,nromDf,nromKHf,nromSf,nromTf,nromUf,nromVf)
  ...
  SUBROUTINE setEle(Xpar,Ypar,n,ele,err,first)
    ...
    LOGICAL :: fst
    INTEGER :: i,triangle,checkele,P_r_ele,P_u_ele,P_v_ele,P_ele,oP_ele,error
    ...
    !write(5,*) P_ele
             !do row=1,297
             ! write(4,*)  (mbathy(row,col),col=1,375)	 
             ! enddo
    !close(5)
...
!       *********************************************************
  ...
  FUNCTION getSlevel(zeta,depth,i,ele,p)

    !This function returns the depth of the current s-level
    USE PARAM_MOD, ONLY: hc,Vtransform
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: i,ele,p
    DOUBLE PRECISION, INTENT(IN) :: zeta,depth
 !   DOUBLE PRECISION, INTENT(IN), OPTIONAL :: E30 !for NEMO
    DOUBLE PRECISION :: S,h,E30,E30w
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: dept,depw, usv,fdept
    INTEGER :: a, b, row, col, mus, ws 
    DOUBLE PRECISION, DIMENSION(51) :: getSlevel
 !  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:),INTENT(out) :: 
    ALLOCATE(dept(51))  !!!!For NEMO 51=ws
    ALLOCATE(depw(51))  !!!!For NEMO 50=us!!!   
    ALLOCATE(fdept(50))  !!!!For NEMO 51=ws
    !ALLOCATE(E30(51))  !!!!For NEMO 51=ws
    !ALLOCATE(E30w(51))  !!!!For NEMO 51=ws
    ALLOCATE(usv(4))  !!!!For NEMO 50=us!!!  
    ...
    SELECT CASE(Vtransform)
      ...
      CASE(4) !NEMO depths from e3t=T-cell thickness

        getSlevel = E3T(i)

      CASE(5) !NEMO reconstruct time verying cell thickness
	                     ! adjust initial vertical scale factors
            !E3T(:,:,i) = E3T0(:,:,i)*( 1._wp + zeta(:,:)*tmask(:,:,1)/(h(:,:) + 1.0 - tmask(:,:,1)) )
            !E3W(:,:,i) = E3W0(:,:,i)*( 1._wp + zeta(:,:)*tmask(:,:,1)/(h(:,:) + 1.0 - tmask(:,:,1)) )
           ! CALL setEle(Xpar,Ypar,n,P_ele)
  ...
  ! open (unit = 5, file = "levels")
    ...
    !write(5,*) depth
   !          do row=1,69575
   !              write(5,*)  (bathy(row,col),col=1,4)	 
   !          enddo
   ! close(5)
         
        do b=1,mus
	    E30 = getInterp("E3T0e",b) !e3t_0 interp to the particle location
 	    E30w = getInterp("E3W0e",b)!Interp of e3w_0
	...
	enddo
	   ...
	   enddo
     !   open (unit = 5, file = "depw")
     !   write(5,*) '% dept='
     !   write(5,*) depw
     !   close(5)
        
	! open (unit = 6, file = "wlevels")
        ! write(6,*) '% E30='
        ! write(6,*) E30w
        ! close(6)
       !   print *, 'E30='
	...
	do a=1,mus-1 !Flip over NEMO
        fdept(a)=DBLE(-1.0) * dept(mus-a)
	enddo
        getSlevel =  fdept
  ...
  FUNCTION getWlevel(zeta,depth,i,ele,p)
    !This function returns the depth of the current w s-level
    USE PARAM_MOD, ONLY: hc,Vtransform
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: i,p,ele
    DOUBLE PRECISION, INTENT(IN) :: zeta,depth
    DOUBLE PRECISION :: S,h,E30,E30w
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: dept,depw, usv, fdepw
    INTEGER :: a, b, row, col,mus,ws    
    DOUBLE PRECISION, DIMENSION(51) :: getWlevel
    ALLOCATE(dept(51))  !!!!For NEMO 51=ws
    ALLOCATE(depw(52))  !!!!For NEMO 50=us!!!   
    ALLOCATE(fdepw(51))  !!!!For NEMO 50=us!!!   
    ALLOCATE(usv(4))  !!!!For NEMO 50=us!!!  
	...
	enddo
	    ...
	    depw(2)=E3T(1)
           ...
           do a=2,mus
              depw(a+1) = depw(a) + E3T(a)
	   enddo

	!write(*,*) E3T(a-2)
	!write(*,*) depw(a-2)
	!do a=2,mus+1 !Flip over NEMO
        !fdepw(a-1)=depw((mus+2)-a)
	!enddo
	do a=1,mus !Flip over NEMO
        fdepw(a)=DBLE(-1.0) * depw((mus+2)-a)
	enddo
  ...
  SUBROUTINE createNetCDF(dob)
    ...
    !NF90_DEF_VAR
       ...
       ! STATUS = NF90_DEF_VAR(NCID,'wdep',NF_DOUBLE,(/timeID/),pageID)
       ! IF(STATUS /= NF90_NOERR) WRITE(*,*) 'Problem createNetCDF: wdep var'
       ! IF(STATUS /= NF90_NOERR) WRITE(*,*) NF_STRERROR(STATUS)
  ...
  SUBROUTINE writeNetCDF(time,age,lon,lat,depth,colors,hitB,hitL,Salt,Temp)
    ...
    IF(STATUS /= NF90_NOERR) WRITE(*,*) NF90_STRERROR(STATUS)
      !wdeps
      !STATUS = NF90_INQ_VARID(NCID, "wdep", modtimeID)
