...
CONTAINS
  ...
  SUBROUTINE gridData(IOSTAT)
    ...
    !INTEGER, ALLOCATABLE, DIMENSION(:,:) :: mbathy !NEMO
    !INTEGER, ALLOCATABLE,DIMENSION(:) :: bathym !NEMO
    !Grid File Output Variables
    INTEGER :: nR,nU,nV,maxR,maxU,maxV,wetR,wetU,wetV,row,col,count
	...
	!for NEMO
    !  ALLOCATE (mbathy(xi_rho,eta_rho),STAT=STATUS)
    !  if(STATUS /= 0) then
    !    write(*,*) 'Problem allocating mbathy'
    !    err = 30 
    !  endif
    ...
    ! READ IN DATA FROM NETCDF FILE TO VARIABLES
     ...
     ! for NEMO variable z levels
    !  STATUS = NF90_INQ_VARID(GF_ID,'mbathy',VID)
    !  STATUS = NF90_GET_VAR(GF_ID,VID,mbathy)
    !  if(STATUS .NE. NF90_NOERR) then
     !   write(*,*)'Problem read mbathy'
      !  err = 40 
     ! endif
